...
public class Pixmaps {
	...
	public static Pixmap scale(Pixmap input, float scalex, float scaley) {
		Pixmap pixmap = new Pixmap((int) (input.getWidth() * scalex), (int) (input.getHeight() * scaley),
				Format.RGBA8888);
	...
	public static Pixmap zoom(Pixmap input, int scale) {
		...
		for (int x = 0; x < pixmap.getWidth(); x++) {
			for (int y = 0; y < pixmap.getHeight(); y++) {
				pixmap.drawPixel(x, y, input.getPixel(x / scale + pixmap.getWidth() / 2 / scale,
						y / scale + pixmap.getHeight() / 2 / scale));
			}
		}
		return pixmap;
	}
	...
	public static Pixmap rotate(Pixmap input, float angle) {
		...
		for (int x = 0; x < input.getWidth(); x++) {
			for (int y = 0; y < input.getHeight(); y++) {
				vector.setX(x - input.getWidth() / 2f + 0.5f);
				vector.setY(y - input.getHeight() / 2f);
				vector = VectorUtils.rotate(vector, -angle);
				int px = (int) (vector.getX() + input.getWidth() / 2f + 0.01f);
				int py = (int) (vector.getY() + input.getHeight() / 2f + 0.01f);
				pixmap.drawPixel(
						px - input.getWidth() / 2 + pixmap.getWidth() / 2,
						py - input.getHeight() / 2 + pixmap.getHeight() / 2,
						input.getPixel(x, y));
			}
		}
	...
	public static boolean isDisposed(Pixmap pix) {
		return (Boolean) NovaGDX.getPrivate(pix, "disposed");
	}
	...
	public static Pixmap huePixmap(int width, int height) {
		...
		for (int x = 0; x < width; x++) {
			ColorUtils.fromHSB(x / (float) width, 1f, 1, color);
			pixmap.setColor(color);
